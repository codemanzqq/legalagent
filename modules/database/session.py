# =============================================================================
# 教学说明：本文件在整体链路中的位置
# 这个文件解决“怎么连 MySQL、怎么拿会话”的问题。  
# 它把连接池和会话生命周期标准化，后面所有数据库读写都统一走它。
# “后面你看到任何模块拿数据库，都是 `get_session_factory()`，这就是统一会话工厂。”
# “后面你看到任何模块写数据库，都是 `session.add()` / `session.commit()`，这就是统一会话。”
